...
def macd_analysis(time_stock_series_df: DataFrame, **kwargs: str) -> bool:
    columns: List[str] = ['Close']

    long_df: DataFrame = time_stock_series_df[columns]\
        .ewm(span=MACD_LONG)\
        .mean()

    time_stock_series_df['macd_closing'] = time_stock_series_df[columns]\
        .ewm(span=MACD_SHORT)\
        .mean() - long_df

    time_stock_series_df['signal_closing'] = time_stock_series_df[columns]\
        .ewm(span=MACD_SIGNAL)\
        .mean() - long_df

    time_stock_series_df['position'] = time_stock_series_df['macd_closing'] > time_stock_series_df['signal_closing']
    time_stock_series_df['pre_position'] = time_stock_series_df['position'].shift(1)
    time_stock_series_df.dropna(inplace=True)
    time_stock_series_df['crossover'] = where(time_stock_series_df['position'] == time_stock_series_df['pre_position'], False, True)
    
    if __name__ == "__main__":
        kwargs['plot_1'][0].plot(time_stock_series_df['Close'], color=kwargs['plot_1'][1])
        kwargs['plot_2'][0].plot(time_stock_series_df['macd_closing'], color=kwargs['plot_2'][1])
        kwargs['plot_3'][0].plot(time_stock_series_df['signal_closing'], color=kwargs['plot_3'][1])
        # time_stock_series_df['Close'].plot()
        # time_stock_series_df[['macd_closing', 'signal_closing']].plot()
...
def analysis(now: Datetime,
    ...
    if __name__ == "__main__":
        print(Datetime.now())
        macd_analysis(time_stock_series_df.copy(), **kwargs)
        print(Datetime.now())
    else:
...
if __name__ == "__main__":
    ...
    figure, plot_1 = pyplot.subplots()
    plot_2 = plot_1.twinx()
    plot_3 = plot_2.twiny()

    color_one = 'tab:red'
    plot_1.set_ylabel('Closing Price', color=color_one)
    plot_1.tick_params(axis='y', labelcolor=color_one)

    color_two = 'tab:blue'
    plot_2.set_ylabel('Index', color=color_two)
    plot_2.tick_params(axis='y', labelcolor=color_two)
    
    color_three = 'springgreen'
    plot_3.set_ylabel('Index', color=color_three)
    plot_3.tick_params(axis='y', labelcolor=color_three)

    analysis(Datetime.now(),
             'TSLA',
             60,
             DataFrame(tesla, columns=['Open', 'High', 'Low', 'Close', 'Volume']),
             plot_1=(plot_1, color_one),
             plot_2=(plot_2, color_two),
             plot_3=(plot_3, color_three))
